...
class ActiveEntity
{
    ...
    public function setInterface(SoftawareInterface $interface){
    }
}
...
class InteractionType
{
    ...
    // private $interactionTypeSet = array();
...
class Sender extends Role
{
    ...
    public function sendMessageToFrom($textMessage, ActiveEntity $toReceiver,ActiveEntity $fromSender){
       if (!empty($textMessage)) {
           $output = "<p style='color: #1c7430'>The sender <b>{SENDER_NAME}</b> sent the message '<b><i>{MESSAGE_TEXT}</i></b>' to the receiver <b>{RECEIVER_NAME}</b></p>";
...
class Goal
{
  ...
  public function __construct($name){
    ...
    /**
     * Return true if gol is achieved else false
     *
     * @return boolean
     */
     ...
     * Set not achieved
     */
    public function setNotAchieved()
    {
        $this->achieved = true;
    }
}
...
class Constraint
{
    ...
    /**
     * Return true if constraint is met else false
     * @return boolean
     */
...
$omcr->setInterface(new MqttInterface($omcr));

$stamec = new ActiveEntity("STAMEC Manufacturing", $sender);
$stamec->setInterface(new MesInterface($stamec));

$message = new Message("Please, provide me an estimation cost for Part Number 01");
...
$dayton->setInterface(new MqttInterface($dayton));
...
$message->setText("Please, provide me an estimation cost for Part Number 02");
...
$omcr->getMessage()->setText("The parts number 01 and 02 you previously required costs, respectively, 1000 and 1020");
$dayton->getMessage()->setText("The parts number 01 and 02 you previously required costs, respectively, 1200 and 1280. Discount of 20% within the end of current month");
...
echo "This can be easily obtained by interchanging roles (by setting Stamec as the receiver and DAYTON,OMCR as senders) <br> and by setting a custom message provided by each senders";
printInteractionTypeInfo($interactionInterchangeRDA);
$communication->activateInteraction($interactionInterchangeRDA);
echo "<hr>";
